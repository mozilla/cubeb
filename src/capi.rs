...
unsafe extern "C" fn capi_get_max_channel_count(c: *mut cubeb::Context, max_channels: *mut u32) -> i32
{
    ...
    match ctx.max_channel_count() {
        Ok(mc) => { *max_channels = mc; cubeb::OK },
        Err(e) => e
    }
}
                                   ...
                                   latency_frames: *mut u32) -> i32
{
    ...
    match ctx.min_latency(&param) {
        Ok(l) => { *latency_frames = l; cubeb::OK },
        Err(e) => e
    }
}
...
unsafe extern "C" fn capi_get_preferred_sample_rate(c: *mut cubeb::Context, rate: *mut u32) -> i32
{
    ...
    match ctx.preferred_sample_rate() {
        Ok(r) => { *rate = r; cubeb::OK },
        Err(e) => e
    }
}

unsafe extern "C" fn capi_get_preferred_channel_layout(c: *mut cubeb::Context, layout: *mut cubeb::ChannelLayout) -> i32
{
    let ctx = &*(c as *mut backend::Context);

    match ctx.preferred_channel_layout() {
        Ok(l) => { *layout = l; cubeb::OK },
        Err(e) => e
    }
}
                                      ...
                                      user_ptr: *mut c_void) -> i32
{
    ...
    match ctx.new_stream(stream_name,
                         input_device,
                         if input_stream_params.is_null() { None } else { Some(*input_stream_params) },
                         output_device,
                         if output_stream_params.is_null() { None } else { Some(*output_stream_params) },
                         latency_frames,
                         data_callback,
                         state_callback,
                         user_ptr as _) {
        Ok(stm) => { *s = Box::into_raw(stm) as *mut _; cubeb::OK }
        Err(e) => e
    }
}
...
unsafe extern "C" fn capi_stream_get_position(s: *mut cubeb::Stream,
                                       position: *mut u64) -> i32
{
    let stm = &*(s as *mut backend::Stream);

    match stm.position() {
        Ok(pos) => { *position = pos; cubeb::OK },
        Err(e) => e
    }
}

unsafe extern "C" fn capi_stream_get_latency(s: *mut cubeb::Stream,
                                      latency: *mut u32) -> i32
{
    let stm = &*(s as *mut backend::Stream);

    match stm.latency() {
        Ok(lat) => { *latency = lat; cubeb::OK },
        Err(e) => e
    }
}
...
unsafe extern "C" fn capi_stream_get_current_device(s: *mut cubeb::Stream, device: *mut *const cubeb::Device) -> i32
{
    ...
    match stm.current_device() {
        Ok(d) => { *device = Box::into_raw(d) as *mut _; cubeb::OK },
        Err(e) => e
    }
}
