...
sun_io_routine(void * arg)
{
  ...
  while (state != CUBEB_STATE_ERROR) {
    ...
    if (s->floating) {
      if (s->record_fd != -1) {
        sun_linear32_to_float(s->record_buf,
      ...
      if (s->play_fd != -1) {
        pthread_mutex_lock(&s->mutex);
        sun_float_to_linear32(s->play_buf,
      ...
      if (s->play_fd != -1) {
        pthread_mutex_lock(&s->mutex);
        sun_linear16_set_vol(s->play_buf, s->p_info.play.channels * to_write, s->volume);
        pthread_mutex_unlock(&s->mutex);
      }
    }
    ...
    while (to_write > 0 || to_read > 0) {
      ...
      if (to_write > 0) {
        bytes = to_write * s->p_frame_size;
        if ((n = write(s->play_fd, s->play_buf + write_ofs, bytes)) < 0) {
          state = CUBEB_STATE_ERROR;
          break;
        }
        frames = n / s->p_frame_size;
        pthread_mutex_lock(&s->mutex);
      ...
      if (to_read > 0) {
        bytes = to_read * s->r_frame_size;
        if ((n = read(s->record_fd, s->record_buf + read_ofs, bytes)) < 0) {
          state = CUBEB_STATE_ERROR;
          break;
        }
        frames = n / s->r_frame_size;
        to_read -= frames;
        read_ofs += frames;
      }
    }
                ...
                void * user_ptr)
{
  ...
  s->p_frame_size = s->p_info.play.channels * (s->p_info.play.precision / 8);
  if (s->play_fd != -1 &&
     (s->play_buf = calloc(SUN_BUFFER_FRAMES, s->p_frame_size)) == NULL) {
    ret = CUBEB_ERROR;
    goto error;
  }
  s->r_frame_size = s->r_info.record.channels * (s->r_info.record.precision / 8);
  if (s->record_fd != -1 &&
     (s->record_buf = calloc(SUN_BUFFER_FRAMES, s->r_frame_size)) == NULL) {
    ret = CUBEB_ERROR;
    goto error;
  }
  *stream = s;
...
sun_stream_get_position(cubeb_stream * s, uint64_t * position)
{
  ...
  *position = (s->blocks_written * s->p_info.blocksize) / s->p_frame_size;
  return CUBEB_OK;
#else
  pthread_mutex_lock(&s->mutex);
...
sun_stream_get_latency(cubeb_stream * s, uint32_t * latency)
{
  ...
  if (ioctl(s->play_fd, AUDIO_GETBUFINFO, &info) == -1) {
    return CUBEB_ERROR;
  }

  *latency = (info.play.seek + info.blocksize) / s->p_frame_size;
  return CUBEB_OK;
#else
